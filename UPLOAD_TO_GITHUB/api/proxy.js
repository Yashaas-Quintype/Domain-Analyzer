...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (service === 'lusha') {
            ...
            headers['api_key'] = process.env.VITE_LUSHA_API_KEY || '34aa0ade-c0d6-4e22-b391-dc281d12d2e1';
        } else if (service === 'zyla') {
            const cleanPath = path || '';
            targetUrl = `https://zylalabs.com/${cleanPath}`;
            headers['Authorization'] = `Bearer ${process.env.VITE_ZYLALABS_KEY || '10095|mmrDs2Whvlc7fD1JKYF2CasMOSaDUZxnVkqhHEzp'}`;
        } else if (service === 'builtwith') {
            targetUrl = `https://api.builtwith.com/${path}`;
            cleanQuery.KEY = process.env.VITE_BUILTWITH_KEY || 'ea894525-80c8-4320-b284-44f5eb507593';
        }
