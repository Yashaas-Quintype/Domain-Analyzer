...
const cleanName = (name) => {
    if (!name) return null;
    const junkWords = ['Privacy', 'Policy', 'Terms', 'Conditions', 'Contact', 'About', 'Home', 'Menu', 'Copyright', 'All Rights Reserved', '©', 'Settings', 'Skip', 'Content', 'Accessibility', 'Help', 'Support', 'Careers', 'Work with us', 'Feedback', 'Sitemap'];
    let cleaned = name;

    // Remove trailing junk words from the end of the name
    for (const word of junkWords) {
        const regex = new RegExp(`[\\s|·•-]+${word}.*$`, 'i');
        cleaned = cleaned.replace(regex, '').trim();
    }

    // Existing prefix cleaning
    let changed = true;
    while (changed) {
        changed = false;
        for (const word of junkWords) {
    ...
    // Final polish: remove non-alphanumeric chars from ends
...
const knowledgeBase = {
    ...
    'nytimes.com': { name: 'The New York Times Company', potentialParentWebsites: ['nytco.com'] }
};
