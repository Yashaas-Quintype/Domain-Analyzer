        ...
        for (let i = 0; i < domainList.length; i += BATCH_SIZE) {
            ...
            await Promise.all(batch.map(async (domain) => {
                try {
                    ...
                    const hasTraffic = (r) => !!(r?.data && (r.data.Engagments?.Visits || (r.data.EstimatedMonthlyVisits && Object.keys(r.data.EstimatedMonthlyVisits).length > 0)));
                    ...
                    const techKey = import.meta.env.VITE_BUILTWITH_KEY || 'ea894525-80c8-4320-b284-44f5eb507593';
                    let techRes = await withRetry(() => axios.get(`/api/builtwith/v22/api.json?KEY=${techKey}&LOOKUP=${domain}`))
                        .catch(e => {
                            console.warn(`BuiltWith fetch failed for ${domain}:`, e.message);
                            return null;
                        });
                    ...
                    if (!techRes?.data?.Results?.[0] && domain.split('.').length > 2) {
                        const root = domain.split('.').slice(-2).join('.');
                        techRes = await withRetry(() => axios.get(`/api/builtwith/v22/api.json?KEY=${techKey}&LOOKUP=${root}`))
                            .catch(() => null);
                    }
